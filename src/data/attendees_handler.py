...
class AttendeesHandler:
    ...
    def register(self, http_request : HttpRequest) -> HttpResponse:
        ...
        if event_attendees_count['amountAttendees'] >= event_attendees_count['maximumAttendees']:
            raise HttpConflict('Evento Lotado!')
    ...
    def find_attendee_badge(self, http_request : HttpRequest) -> HttpResponse:
        ...
        print(f'1---> {attendee_id}')
        badge = self.__attendees_repository.attendee_get_by_id(attendee_id)
    ...
    def find_attendees_from_event_id(self, http_request : HttpRequest) -> HttpResponse:
        event_id = http_request.param['event_id']
        attendees = self.__attendees_repository.get_attendees_by_event_id(event_id)
        if not len(attendees): raise HttpNotFound('Participantes não encontrados!')
